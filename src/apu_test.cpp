...
void APU::run_tests() {
	...
	const std::vector<std::tuple<uint16_t, uint8_t, uint8_t>> values = {
		/* --- Check min values (all 0). This tests whether the read filter
		works
		--- */
		// NR10-NR14
		t(0xFF10, 0x00, 0x80),
	...
	for (const auto& entry : values) {
		...
		if (real_read != expected_read) {
			std::cout << std::hex << "TEST FAILED: Addr 0x"
					  << (unsigned int)addr << " - wrote "
					  << (unsigned int)write << ", expected "
					  << (unsigned int)expected_read << ", got "
					  << (unsigned int)real_read << std::dec << "\n";
		}
	}
}
