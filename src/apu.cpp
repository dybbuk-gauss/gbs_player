...
APU::APU(SystemClock& clock) : frame_sequencer(256), power_on(true) {
	...
	regwrite_ptr = {
		&Channel::NRx0_write, &Channel::NRx1_write, &Channel::NRx2_write,
		&Channel::NRx3_write, &Channel::NRx4_write,
	};

	regread_ptr = {
		&Channel::NRx0_read, &Channel::NRx1_read, &Channel::NRx2_read,
		&Channel::NRx3_read, &Channel::NRx4_read,
	};
...
void APU::register_write(uint16_t addr, uint8_t value) {
	...
	if (addr < 0xFF15)
		(dynamic_cast<Channel*>(&square1)->*regwrite_ptr[i])(value);
	else if (addr < 0xFF1A)
		(dynamic_cast<Channel*>(&square2)->*regwrite_ptr[i])(value);
	else if (addr < 0xFF1F)
		(dynamic_cast<Channel*>(&wave)->*regwrite_ptr[i])(value);
	else if (addr < 0xFF24)
		(dynamic_cast<Channel*>(&noise)->*regwrite_ptr[i])(value);
	else if (addr < 0xFF27) {
		if (i == 0)
			NR50_write(value);
		else if (i == 1)
		...
		// same address. So we need to shift the value & double the address
		// offset
...
uint8_t APU::register_read(uint16_t addr) {
	...
	uint8_t i = (addr - 0xFF10) % 5;
	if (addr < 0xFF15)
		return (dynamic_cast<Channel*>(&square1)->*regread_ptr[i])() |
			   register_mask[0][i];
	else if (addr < 0xFF1A)
		return (dynamic_cast<Channel*>(&square2)->*regread_ptr[i])() |
			   register_mask[1][i];
	else if (addr < 0xFF1F)
		return (dynamic_cast<Channel*>(&wave)->*regread_ptr[i])() |
			   register_mask[2][i];
	else if (addr < 0xFF24)
		return (dynamic_cast<Channel*>(&noise)->*regread_ptr[i])() |
			   register_mask[3][i];
	else if (addr < 0xFF27) {
		if (i == 0)
			return NR50_read() | register_mask[4][i];
		else if (i == 1)
			return NR51_read() | register_mask[4][i];
		else if (i == 2)
			return NR52_read() | register_mask[4][i];
	} else if (addr >= 0xFF30 && addr <= 0xFF3F) {
...
void APU::boot_sound() {
	...
	register_write(0xFF25,
				   0xF3); // Enable both left/right output for square channels
...
uint8_t APU::NR50_read() {
	...
	return (vin_left << 7) | (volume_left << 4) | (vin_right << 3) |
		   volume_right;
}
...
uint8_t APU::NR51_read() {
	...
	return (noise_left << 7) | (wave_left << 6) | (square2_left << 5) |
		   (square1_left << 4) | (noise_right << 3) | (wave_right << 2) |
...
uint8_t APU::NR52_read() {
	...
	return (power_on << 7) | (noise_enabled << 3) | (wave_enabled << 2) |
		   (square2_enabled << 1) | square1_enabled;
}
