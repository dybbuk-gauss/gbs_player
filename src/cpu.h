...
class CPU {
  ...
  private:
	...
	// Important: the 16-bit value is big-endian here, which means that raw
	// values read from code must be converted first
	...
	uint16_t inc(uint16_t val); // it's easier with the 16 bit regs if the value
	// is returned
