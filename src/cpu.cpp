...
CPU::CPU(APU& apu)
	...
	opcode_cycles = {
		...
		12, 12, 4, 12, 16, 8,  16, 8,  16, 12, 4, 12, 4,  8,  16, 12, 12, 8,  4,
		4,	16, 8, 16, 16, 4,  16, 4,  4,  4,  8, 16, 12, 12, 8,  4,  4,  16, 8,
		16, 12, 8, 16, 4,  4,  4,  8,  16};
	...
	opcode_cycles_branch = {
		4,	12, 8,	8,	4,	4,	8,	4,	20, 8,	8,	8,	4,	4,	8,	4,	4,	12,
		8,	8,	4,	4,	8,	4,	12, 8,	8,	8,	4,	4,	8,	4,	12, 12, 8,	8,
		4,	4,	8,	4,	12, 8,	8,	8,	4,	4,	8,	4,	12, 12, 8,	8,	12, 12,
		12, 4,	12, 8,	8,	8,	4,	4,	8,	4,	4,	4,	4,	4,	4,	4,	8,	4,
		4,	4,	4,	4,	4,	4,	8,	4,	4,	4,	4,	4,	4,	4,	8,	4,	4,	4,
		4,	4,	4,	4,	8,	4,	4,	4,	4,	4,	4,	4,	8,	4,	4,	4,	4,	4,
		4,	4,	8,	4,	8,	8,	8,	8,	8,	8,	4,	8,	4,	4,	4,	4,	4,	4,
		8,	4,	4,	4,	4,	4,	4,	4,	8,	4,	4,	4,	4,	4,	4,	4,	8,	4,
		4,	4,	4,	4,	4,	4,	8,	4,	4,	4,	4,	4,	4,	4,	8,	4,	4,	4,
		4,	4,	4,	4,	8,	4,	4,	4,	4,	4,	4,	4,	8,	4,	4,	4,	4,	4,
		4,	4,	8,	4,	4,	4,	4,	4,	4,	4,	8,	4,	20, 12, 16, 16, 24, 16,
		8,	16, 20, 16, 16, 4,	24, 24, 8,	16, 20, 12, 16, 4,	24, 16, 8,	16,
		20, 16, 16, 4,	24, 4,	8,	16, 12, 12, 8,	4,	4,	16, 8,	16, 16, 4,
		16, 4,	4,	4,	8,	16, 12, 12, 8,	4,	4,	16, 8,	16, 12, 8,	16, 4,
		4,	4,	8,	16,
	};
...
void CPU::gbs_load(uint16_t load_addr, std::vector<uint8_t>& data) {
	...
	// Check whether the data to be loaded all fits into the two rom banks or
	// not
...
void CPU::load_test_rom(std::string filename) {
	...
	file_buf.insert(file_buf.begin(), std::istream_iterator<uint8_t>(file),
...
bool CPU::procedure_done() { return state.sp > sp_start; }
...
bool CPU::is_stopped() { return stopped; }
...
void CPU::clear_ram() { std::memset(&state.memory[0xA000], 0, 0x4000); }

void CPU::clear_memory() { state.memory.fill(0); }
...
void CPU::memory_write(uint16_t addr, uint8_t value) {
	...
	if (addr == 0xA000 && value != 0x80)
		if (memory_read(0xA001) == 0xDE && memory_read(0xA002) == 0xB0 &&
...
void CPU::set_AF(uint16_t value) { set_AF((value >> 8) & 0xFF, value & 0xFF); }

void CPU::set_BC(uint16_t value) { set_BC((value >> 8) & 0xFF, value & 0xFF); }

void CPU::set_DE(uint16_t value) { set_DE((value >> 8) & 0xFF, value & 0xFF); }

void CPU::set_HL(uint16_t value) { set_HL((value >> 8) & 0xFF, value & 0xFF); }
...
uint16_t CPU::get_AF() {
	uint8_t f = ((state.f.z << 7) | (state.f.n << 6) | (state.f.h << 5) |
				 (state.f.c << 4)) &
				0xF0;
	return (state.a << 8) | f;
}

uint16_t CPU::get_BC() { return (state.b << 8) | (state.c); }

uint16_t CPU::get_DE() { return (state.d << 8) | (state.e); }

uint16_t CPU::get_HL() { return (state.h << 8) | (state.l); }
...
void CPU::res(uint8_t bit, uint8_t& reg) { reg &= ~(1 << bit); }

void CPU::set(uint8_t bit, uint8_t& reg) { reg |= (1 << bit); }
...
uint16_t CPU::inc(uint16_t val) { return val + 1; }

uint16_t CPU::dec(uint16_t val) { return val - 1; }
